...
public class BST<T extends Comparable<? super T>> implements Iterable<T>{
	...
	/**
	 * Returns the specified element in the BST. More formally, gets an element e such that element.compareTo(e) == 0, if this BST contains such an element
	 * @param element -
	...
	private class BSTIterator implements Iterator<T> {
		...
		public T next() {
			...
			node = stack.pop();
			T tmpElement = node.element;
			
			
			
			if(node.right != EMPTY_NODE) {
